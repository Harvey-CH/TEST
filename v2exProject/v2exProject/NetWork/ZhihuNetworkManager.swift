...
class ZhihuNetworkManager: NSObject {
    ...
    func fetchData(api:ZhihuAPI,success:(value:AnyObject)->(),failure:(message:String)->()) {
        ...
        Alamofire.request(.GET, (baseURL + api.rawValue)).responseJSON { (request, response, result) -> Void in
            switch result {
...
//        Alamofire.request(.GET, (baseURL + api.rawValue),parameters: [:]).responseJSON { (response) in
